...
class resnet_proto_IoU(nn.Module):
    def __init__(self, opt):
        ...
        if opt.dataset == 'CUB':
            ...
            for name in self.extract:
                ...
                self.prototype_vectors[name] = nn.Parameter(2e-4 * torch.rand(prototype_shape), requires_grad=True) #原型参数
            self.prototype_vectors = nn.ParameterDict(self.prototype_vectors)
    ...
    def forward(self, x, attribute, return_map=False):
        ...
        output_final = self.softmax(pre_attri['final'].mm(attribute))  #[batchsize,类别数]
        ...
        for name in self.extract:
            ...
            attention[name] = F.conv2d(input=record_features[name], weight=self.prototype_vectors[name])  # [64, 312, 7, 7]
            # attention[name] = torch.einsum('bfmn,vf->bvmn',record_features[name],self.prototype_vectors[name])

            #### 最大化，得到 a
            pre_attri[name] = F.max_pool2d(attention[name], kernel_size=self.kernel_size[name]).view(batch_size, -1) # [64, 312]
            #### 得到分类结果
